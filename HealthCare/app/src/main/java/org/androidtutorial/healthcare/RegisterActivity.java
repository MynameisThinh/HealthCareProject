                ...
                if(username.length() == 0 || email.length() == 0 || password.length() == 0 || confirm.length() == 0){
                    ...
                    if(password.compareTo(confirm) == 0){
                        if(isValid(password)){
                            ...
                            Toast.makeText(getApplicationContext(), "Mật khẩu phải ít nhất 8 ký tự,phải chứa in hoa, số và kí tự đặc biệt",Toast.LENGTH_SHORT).show();
                        }
                    }else {
                        Toast.makeText(getApplicationContext(), "Mật khẩu và xác nhận mật khẩu không khớp",Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
